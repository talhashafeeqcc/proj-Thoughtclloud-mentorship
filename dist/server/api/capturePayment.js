...
import { getDocument, getDocuments, updateDocument, whereEqual, COLLECTIONS } from '../../services/firebase';
...
export const capturePaymentHandler = async (req, res) => {
    try {
        ...
        const payments = await getDocuments(COLLECTIONS.PAYMENTS, [whereEqual('paymentIntentId', paymentIntentId)]);
        if (!payments || payments.length === 0) {
            return res.status(200).json(capturedPayment);
        }
        const payment = payments[0];
        // Get the mentor's Stripe account ID
        const mentor = await getDocument(COLLECTIONS.MENTORS, payment.mentorId);
