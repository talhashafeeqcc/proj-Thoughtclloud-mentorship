...
import { getMentors, getMentorById } from '../services/userService';
...
// Reducer function
const mentorReducer = (state: MentorState, action: MentorAction): MentorState => {
  switch (action.type) {
    ...
    case 'FETCH_MENTORS_SUCCESS':
      return {
        ...
        filteredMentors: state.filter ? applyFilter(action.payload, state.filter) : action.payload,
        loading: false,
      };
...
const applyFilter = (mentors: Mentor[], filter: MentorFilter): Mentor[] => {
  return mentors.filter((mentor) => {
    ...
    if (filter.priceRange) {
      if (
        (filter.priceRange.min !== undefined && mentor.sessionPrice < filter.priceRange.min) ||
        (filter.priceRange.max !== undefined && mentor.sessionPrice > filter.priceRange.max)
      ) {
        return false;
      }
    }
    ...
    if (filter.availability && filter.availability.date) {
      const availableOnDate = mentor.availability.some(
        slot => 
          ...
          (!filter.availability!.startTime || slot.startTime >= filter.availability!.startTime) &&
          (!filter.availability!.endTime || slot.endTime <= filter.availability!.endTime) &&
          !slot.isBooked
      );
      if (!availableOnDate) {
        return false;
...
export const MentorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchMentors = async () => {
    ...
    try {
      ...
      dispatch({
        type: 'FETCH_ERROR',
        payload: error instanceof Error ? error.message : 'Failed to fetch mentors',
      });
    }
  };
  ...
  const fetchMentorById = async (id: string) => {
    ...
    try {
      ...
      dispatch({
        type: 'FETCH_ERROR',
        payload: error instanceof Error ? error.message : `Failed to fetch mentor with ID ${id}`,
      });
    }
  };
  ...
  return <MentorContext.Provider value={contextValue}>{children}</MentorContext.Provider>;
};
